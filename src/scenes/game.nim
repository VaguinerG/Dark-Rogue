...
proc drawMap() =
    let
        mapTexture = addr MAP_LEVELS[SELECTED_MAP]
        textureWidth = mapTexture.width.float32
        textureHeight = mapTexture.height.float32
    
    let
        startX = int((camera.target.x - camera.offset.x) / textureWidth) - 1
        startY = int((camera.target.y - camera.offset.y) / textureHeight) - 1
        endX = int((camera.target.x + camera.offset.x) / textureWidth) + 1
        endY = int((camera.target.y + camera.offset.y) / textureHeight) + 1
